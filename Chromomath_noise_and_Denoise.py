...
# def generate_gaussian_noise(image, intensity):
#     # вычисление стандартного отклонения гауссовского распределения
#     # в зависимости от интенсивности шума и диапазона значений пикселей
#
#     max = np.max(image)  # максимальное значение пикселя
#     min = np.min(image)  # минимальное значение пикселя
#     std = intensity * (max - min) / 2
#
#     # создание матрицы случайных значений по стандартному нормальному распределению
#     # с помощью np.random.normal() и заданным std и размером изображения
#     noise_matrix = np.random.normal(0, std, image.shape)
#
#     # сложение исходного изображения с матрицей шума поэлементно с учетом переполнения с помощью np.add()
#     noisy_image = np.add(image, noise_matrix)
#     return noisy_image
...
def generate_salt_and_pepper_noise(image, intensity):
    ...
    # генерируем случайную матрицу с размером изображения и значениями от 0 до 1
    random_matrix = np.random.rand(height, width)
    # находим индексы пикселей, которые будут заменены на черный цвет
    black_indices = random_matrix < intensity / 2
    # находим индексы пикселей, которые будут заменены на белый цвет
    white_indices = random_matrix > 1 - intensity / 2
    # заменяем пиксели на черный цвет по индексам
    noisy_image[black_indices] = [0, 0, 0]
    # заменяем пиксели на белый цвет по индексам
    noisy_image[white_indices] = [255, 255, 255]
...
mode_switch.pack(side=tk.TOP)
mode_switch = tk.Radiobutton(top_frame, text="Нелокальное усреднение", value=4, variable=mode_var, command=switch_mode)
mode_switch.pack(side=tk.TOP)
